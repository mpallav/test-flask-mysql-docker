    ...
    try:
        ...
        for i in range(len(ts_collect) - 1):
            ...
            if i > 0:
                ...
                time1 = datetime.datetime.strptime(ts_collect[i-1], "%b %d %H:%M:%S")
...
def utcTOpstconvertor(filename):
    newlog = []
    '''get the timestamp'''
#    pst = timezone('US/Pacific')
    for timestamp in filename:
         ts = re.search('(^\w{3}\s\d{2}\s\d{2}\:\d{2}\:\d{2}).+(\d{2}\/\w{3}\/\d{4}\:\d{2}:\d{2}:\d{2})',timestamp)
...
#    print data 
    ...
    collect_ts = timeElapsed(data)
    print "Average time elapsed"
    print collect_ts
 
    print_log = utcTOpstconvertor(data)
    print print_log
